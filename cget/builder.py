...
class Builder:
    ...
    def fetch(self, url, hash=None, copy=False, insecure=False, retries=3, retry_delay=300):
        self.prefix.log("fetch:", url)
        if insecure: url = url.replace('https', 'http')
        f = util.retrieve_url(url, self.top_dir, copy=copy, insecure=insecure, hash=hash, retries=retries, retry_delay=retry_delay)
        if os.path.isfile(f):
